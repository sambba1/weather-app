...
export default function ListLocations(props) {
    ...
    const addLocation = async (newLocation) => {
        ...
        if (isValidCity) {
            const newId = props.locations.length > 0 ? Math.max(...props.locations.map(location => location.id)) + 1 : 1;
            const newLocationData = { id: newId, city: newLocation, weather: { main: { temp: "-" }, weather: [{ description: '-' }] } };
            props.setLocations([...props.locations, newLocationData]);
        } else {
    ...
    const cityText = (city) => {
        if (city == "gps") {
            return (
                <Text style={{ fontSize: 40, flex: 6 }}>Oma sijainti</Text>
            )
        } else {
            return (
                <Text style={{ fontSize: 40, flex: 6 }}>{city}</Text>
            )
        }
    }
    ...
    const renderItem = ({ item }) => (
        <View style={{
            ...
            <View style={{ flex: 5, flexDirection: 'column' }} >
                <Text style={{ fontSize: 20, flex: 1 }}>{item.weather.main.temp + "°"}</Text>
                <Text style={{ fontSize: 20, flex: 1 }}>{item.weather.weather[0].description}</Text>
            </View>
            <View style={{ flex: 2 }}>
                {deleteButton(item)}
            </View>
